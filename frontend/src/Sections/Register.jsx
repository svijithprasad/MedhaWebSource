...
const Register = () => {
  ...
  const calculateTotalAmount = () => {
    ...
    Object.keys(events).forEach((event) => {
      if (events[event]) {
        const participants = eventDetails[event];
        technicalParticipants += Object.values(participants).filter((p) => p.trim() !== '').length;
      }
    });
    ...
    Object.keys(culturalEvents).forEach((event) => {
      if (culturalEvents[event]) {
        const participants = eventDetails[event];
        culturalParticipants += Object.values(participants).filter((p) => p.trim() !== '').length;
      }
    });
    ...
    let selectedTechnicalEventsCount = Object.values(events).filter(event => event).length;
